...
def get_service_profile_templates(ucs_domain, ucs_username, ucs_password):
    ...
    try:
        ...
        # Filter to get only the initial and updating templates
        sp_initial_template_names = [sp.name for sp in sp_templates if sp.type == "initial-template"]
        sp_updating_template_names = [sp.name for sp in sp_templates if sp.type == "updating-template"]
        
        # Combine both lists
        combined_templates = sp_initial_template_names + sp_updating_template_names
...
if __name__ == "__main__":
    ...
    combined_template_names = get_service_profile_templates(ucs_domain, ucs_username, ucs_password)

    # Print the list of Service Profile Templates
    print("Service Profile Templates:")
    for template_name in combined_template_names:
        print(template_name)
